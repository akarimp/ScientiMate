def hurricanewindh80(xgrid, ygrid, xCenter, yCenter, Pc, Vgmax, Rknown, VgRknown, Rmax=423e3, Pn=101325, Rhoa=1.204, \
    ...
    * www.aoml.noaa.gov/hrd/data_sub/re_anal.html
    ...
    for i in range(0,len(xCenter),1):
        ...
        VgR=(B[i]/Rhoa*(RVmax[i]/Rgrid[:,:,i])**B[i]*deltaP[i]*np.exp(-(RVmax[i]/Rgrid[:,:,i])**B[i]))**0.5 #Gradient wind velocity at radius R without coriolis parameter
        #VgR=(B[i]/Rhoa*(RVmax[i]/Rgrid[:,:,i])**B[i]*deltaP[i]*np.exp(-(RVmax[i]/Rgrid[:,:,i])**B[i])+(Rgrid[:,:,i]*f/2)**2)**0.5-(Rgrid[:,:,i]*f/2) #Gradient wind velocity at radius R with coriolis parameter
    ...
    #--------------------------------------------------------------------------
    #Outputs
    return Vxgrid, Vygrid, Vgrid, Vgxgrid, Vgygrid, Vggrid, Pgrid, Rgrid, RVmax, thetagrid, thetaVgrid, thetaVgridtan
