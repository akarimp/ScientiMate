def hurricanewindh08(xgrid, ygrid, xCenter, yCenter, Pc, Vmax, Rknown, VgRknown, dPcdt, Vt, Rmax=423e3, Pn=101325, Rhoa=1.204, SST=None, \
    ...
    * www.aoml.noaa.gov/hrd/data_sub/re_anal.html
    ...
    if SST is None: SST=(28-3*(np.abs(yCenter)-10)/20)+1
    ...
    RVmaxs=np.zeros(len(xCenter)); #Pre-assigning array
    ...
    for i in range(0,len(xCenter),1):
        ...
        RVmaxini=Rknown[i]*(VgRknown[i]/Vmax[i])**2; #Initial guess
    
        #Calculating radius of maximum wind from known velocity and its radius
        fun = lambda x : (VgRknown[i]-(B[i]/Rhoa*(x/Rknown[i])**B[i]*deltaP[i]*np.exp(-(x/Rknown[i])**B[i]))**0.5); #Hurricane radius for Vmax without coriolis parameter
    ...
    bs=-4.4e-5*deltaPshPa**2+0.01*deltaPshPa+0.03*dPcdt-0.014*np.abs(yCenter)+0.15*Vt**xs+1; #Hurricane shape parameter at surface
    Vmaxs=(bs*deltaP/(Rhoas*np.exp(1)))**0.5; #Hurricane maximum wind velocity at surface level (10m above surface)
    ...
    for i in range(0,len(xCenter),1):
        ...
        #Rankine vortex model for R>=RVmax: V=Vmax*(RVmax/R)^x, where 0.4<x<0.6 (e,g. Phadke et al., 2003; Holland et al., 2010)
        #Rankine vortex model for R<RVmax: V=Vmax*(R/RVmax)^x, where 0.4<x<0.6 (e,g. Phadke et al., 2003; Holland et al., 2010)
        #RVmaxini=Rknown[i]*(VRknown[i]/Vmaxs[i])**2 #Initial guess
    
        #Calculating radius of maximum wind from known velocity and its radius
        #fun = lambda x : (VRknown[i]-(bs[i]/Rhoas[i]*(x/Rknown[i])**bs[i]*deltaP[i]*np.exp(-(x/Rknown[i])**bs[i]))**xs[i]); #Hurricane radius for Vmax without coriolis parameter
        #RVmaxs[i]=sp.optimize.fsolve(fun,RVmaxini); #Radius of maximum wind
        RVmaxs[i]=RVmax[i];
    
        #Surface wind velocity at 10 m height at radius R
        #VR=(bs[i]/Rhoas[i]*(RVmaxs[i]/Rgrid[:,:,i])**bs[i]*deltaP[i]*np.exp(-(RVmaxs[i]/Rgrid[:,:,i])**bs[i]))**xs[i]; #Gradient wind velocity at radius R without oriolis parameter
        VR=Vmaxs[i]*((RVmaxs[i]/Rgrid[:,:,i])**bs[i]*np.exp(1-(RVmaxs[i]/Rgrid[:,:,i])**bs[i]))**xs[i]; #Gradient wind velocity at radius R without oriolis parameter
    ...
    Vxgrid=Vgrid*np.cos(np.deg2rad(thetaVgrid)); #Hurricane surface velocity in x (East) direction
    Vygrid=Vgrid*np.sin(np.deg2rad(thetaVgrid)); #Hurricane surface velocity in y (North) direction
    ...
    #--------------------------------------------------------------------------
    #Outputs
    return Vxgrid, Vygrid, Vgrid, Pgrid, Rgrid, RVmax, thetagrid, thetaVgrid, thetaVgridtan
