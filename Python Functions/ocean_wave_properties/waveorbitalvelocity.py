def waveorbitalvelocity(h, H, T, z, kCalcMethod='beji'):
    """
    .. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
    .. +                                                                        +
    .. + ScientiMate                                                            +
    ...
    Uw=(np.pi*H*np.cosh(k*(z+h)))/(T*np.sinh(k*h)) #Maximum wave orbital veocity at elevation z 
    A=Uw*T/(2*np.pi) #Maximum wave orbital excursion at elevation z 
    
    Uwb=(np.pi*H)/(T*np.sinh(k*h)) #Wave orbital veocity at bed 
    Ab=Uwb*T/(2*np.pi) #Wave orbital excursion at the bed using linear wave theory
