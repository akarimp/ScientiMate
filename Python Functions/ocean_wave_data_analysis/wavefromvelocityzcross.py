def wavefromvelocityzcross(Ux, Uy, fs, h, heightfrombed=0, Kuvmin=0.15, kCalcMethod='beji', dispout='no'):
    """
    .. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
    .. +                                                                        +
    .. + ScientiMate                                                            +
    ...
    U=np.sqrt(UxDetrended**2+UyDetrended**2)
    U=U*np.sign(UxDetrended)
    ...
    N=len(Ux) #Length of a time series, total number of points in input file, N=fs*duration
    ...
    if ((UDetrended[0]==0) and (UDetrended[1]>0)): 
        len3=1

    if (((UDetrended[0]==0) and (UDetrended[1]<0)) or (UDetrended[0]!=0)):
        for i in range(0,N-1,1):
            if ((UDetrended[i]<0) and (UDetrended[i+1]>0)): 
                len3=i
                break

    #Detecting the end point of last wave (last complete crest-trough cycle)
    if ((UDetrended[-1]==0) and (UDetrended[-2]<0)):
        len4=N

    if (((UDetrended[-1]==0) and (UDetrended[-2]>0)) or (UDetrended[-1]!=0)):
    ...
    for i in range(len3,len4+1,1): 
        ...
        if ((i>1) and (i<N)):
            if ((UDetrended[i]<0) and (UDetrended[i+1]>0)):
                m=m+1
                xupcross=np.append(xupcross,t[i]-(t[i+1]-t[i])/(UDetrended[i+1]-UDetrended[i])*UDetrended[i])
                yupcross=np.append(yupcross,0)
                xupcrossIndx=np.append(xupcrossIndx,i)
        ...
        if ((i>1) and (i<N)):
            if ((UDetrended[i]>0) and (UDetrended[i+1]<0)):
                n=n+1
                xdowncross=np.append(xdowncross,t[i]-(t[i+1]-t[i])/(UDetrended[i+1]-UDetrended[i])*UDetrended[i])
    ...
    for i in range(xupcrossIndx[0],xupcrossIndx[-1]+1,1):
        ...
        if ((i>1) and (i<N)):
            ...
            if ((UDetrended[i]>UDetrended[i-1]) and (UDetrended[i]>UDetrended[i+1])):
    ...
    Kuv=(2*np.pi/T)*np.cosh(k*heightfrombed)/np.sinh(k*h)
    Kuv[0]=Kuv[1]
    ...
    for i in range(xupcrossIndx[0],xupcrossIndx[-1]+1,1):
        ...
        if ((i>1) and (i<N)):
            ...
            if ((EtawithKuv[i]>EtawithKuv[i-1]) and (EtawithKuv[i]>EtawithKuv[i+1])):
                ...
                if EtawithKuv[i]>0:
                    ...
                    if m==n:
                        ...
                        m=m+1
                        xmax=np.append(xmax,t[i])
                        ymax=np.append(ymax,EtawithKuv[i])
                        xmaxIndx=np.append(xmaxIndx,i)

                    elif m!=-1:
                            
                        #Replacingthe old crest location with new one if the new one is larger
                        if ((EtawithKuv[i]>ymax[m]) and (m!=-1)):
            ...
            if ((EtawithKuv[i]<EtawithKuv[i-1]) and (EtawithKuv[i]<EtawithKuv[i+1])):
                ...
                if EtawithKuv[i]<0:
                    ...
                    if n==m-1:
                        ...
                        n=n+1
                        xmin=np.append(xmin,t[i])
                        ymin=np.append(ymin,EtawithKuv[i])
                        xminIndx=np.append(xminIndx,i)
                        
                    elif n!=-1:
                            
                        #Replacingthe old crest location with new one if the new one is smaller
                        if EtawithKuv[i]<ymin[n]:
    ...
    if dispout=='yes':
        ...
        plt.plot(t,Eta,label='Time Series')
    ...
    #--------------------------------------------------------------------------
    #Outputs
    return Hs, Ts, Hz, Tz, Hrms, H, T, Eta, t
