def wavefrompressurezcross(P, fs, h, heightfrombed=0, Kpmin=0.15, kCalcMethod='beji', Rho=1000, dispout='no'):
    """
    .. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
    .. +                                                                        +
    .. + ScientiMate                                                            +
    ...
    for i in range(xupcrossIndx[0],xupcrossIndx[-1]+1,1):
        ...
        if ((i>1) and (i<N)):
            ...
            if ((EtawithKp[i]>EtawithKp[i-1]) and (EtawithKp[i]>EtawithKp[i+1])):
                ...
                if EtawithKp[i]>0:
                    ...
                    if m==n:
                        ...
                        m=m+1
                        xmax=np.append(xmax,t[i])
                        ymax=np.append(ymax,EtawithKp[i])
                        xmaxIndx=np.append(xmaxIndx,i)

                    elif m!=-1:
                            
                        #Replacingthe old crest location with new one if the new one is larger
                        if ((EtawithKp[i]>ymax[m]) and (m!=-1)):
            ...
            if ((EtawithKp[i]<EtawithKp[i-1]) and (EtawithKp[i]<EtawithKp[i+1])):
                ...
                if EtawithKp[i]<0:
                    ...
                    if n==m-1:
                        ...
                        n=n+1
                        xmin=np.append(xmin,t[i])
                        ymin=np.append(ymin,EtawithKp[i])
                        xminIndx=np.append(xminIndx,i)
                        
                    elif n!=-1:
                            
                        #Replacingthe old crest location with new one if the new one is smaller
                        if EtawithKp[i]<ymin[n]:
