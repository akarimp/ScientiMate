function [H, T, Ehat, fphat, m0, Fetchhat] = parametricwavedeep(windvel, Fetch, CalcMethod, dispout)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
if strcmp(CalcMethod,'wilson')==1
    ...
    Tshat=(1.37*2*pi).*(1-(1+0.008.*Fetchhat.^(1/3)).^(-5)); %Dimensionless significant wave period: Tshat=g*Ts/U10
    ...
    %Hm0FullyDev=4.*sqrt(m0FullyDev); %Zero-moment wave height for fully developed condition for fully developed condition
    %Hm0hatFullyDev=9.81.*Hm0FullyDev./(windvel.^2); %Dimensionless zero-moment wave height for fully developed condition 
    Hm0hatFullyDev=4*sqrt(3.64e-3); %Dimensionless zero-moment wave height for fully developed condition 
...
elseif strcmp(CalcMethod,'jonswap')==1
    ...
    Hm0=4.*sqrt(m0); %Zero-moment wave height, Hm0=4*(m0)^0.5

    fp=fphat.*9.81./(windvel); %Peak wave frequency, fphat=fp*U10/g
    Tp=1./fp; %Peak wave period, Tp=1/fp

    %Assigning wave propoerties
    H=Hm0; %Peak wave height
    T=Tp; %Peak wave period
...
elseif strcmp(CalcMethod,'kahma')==1
    ...
    fphat=(13.7./(2*pi)).*Fetchhat.^(-0.27); %Dimensionless peak wave frequency: fphat=fp*U10/g
...
elseif strcmp(CalcMethod,'hwang')==1
    ...
    fphat=(11.86./(2*pi)).*Fetchhat.^(-0.2368); %Dimensionless peak wave frequency: fphat=fp*U10/g
...
if strcmp(dispout,'yes')==1
    ...
    %Plotting
    subplot(1,2,1)
    loglog(Fetchhat,Ehat)

    xlabel('Dimensionless Wind Fetch, Fetchhat=g*Fetch/U10^2')
    ylabel('Dimensionless wave energy, Ehat=g^2*m0/U10^4')

    subplot(1,2,2)
    loglog(Fetchhat,fphat)

    xlabel('Dimensionless Wind Fetch, Fetchhat=g*Fetch/U10^2')
    ylabel('Dimensionless peak wave frequency, fphat=fp*U10/g')
