function [H, T, Ehat, fphat, m0, Fetchhat, hhat] = parametricwaveshallow(windvel, Fetch, hmean, CalcMethod, dispout)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
elseif strcmp(CalcMethod,'young')==1
    ...
    Ehat=3.64e-3.*(tanh(0.493.*(hhat).^(0.75)).*tanh((0.00313.*(Fetchhat).^(0.57))./(tanh(0.493.*(hhat).^(0.75))))).^1.74; %Dimensionless wave energy: Ehat=g^2*m0/U10^4
    fphat=0.133.*(tanh(0.331.*(hhat).^(1.01)).*tanh((5.215e-4.*(Fetchhat).^(0.73))./(tanh(0.331.*(hhat).^(1.01))))).^-0.37; %Dimensionless peak wave frequency: fphat=fp*U10/g
