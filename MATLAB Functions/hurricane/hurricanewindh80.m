function [Vxgrid, Vygrid, Vgrid, Vgxgrid, Vgygrid, Vggrid, Pgrid, Rgrid, RVmax, thetagrid, thetaVgrid, thetaVgridtan] = hurricanewindh80(xgrid, ygrid, xCenter, yCenter, Pc, Vgmax, Rknown, VgRknown, Rmax, Pn, Rhoa, ...
...
switch nargin
    case 8
        Rmax=423e3; Pn=101325; Rhoa=1.204; VgToVCoeff=0.8; inflowdirCalcMethod='bretschneider'; Vt=0; VtAzmdir=0; backwindCalcMethod='no'; distCalcMethod='gc'; flattendata='no'; savedata='no'; dispout='no';
    case 9
        Pn=101325; Rhoa=1.204; VgToVCoeff=0.8; inflowdirCalcMethod='bretschneider'; Vt=0; VtAzmdir=0; backwindCalcMethod='no'; distCalcMethod='gc'; flattendata='no'; savedata='no'; dispout='no';
    case 10
        Rhoa=1.204; VgToVCoeff=0.8; inflowdirCalcMethod='bretschneider'; Vt=0; VtAzmdir=0; backwindCalcMethod='no'; distCalcMethod='gc'; flattendata='no'; savedata='no'; dispout='no';
    case 11
...
for i=1:length(xCenter(:,1))
    ...
    %fun = @(x)(VgRknown(i,1)-(B(i,1)/Rhoa*(x/Rknown(i,1))^B(i,1)*deltaP(i,1)*exp(-(x/Rknown(i,1))^B(i,1))+(Rknown(i,1)*f(i,1)/2)^2).^0.5-(Rknown(i,1)*f(i,1)./2)); %Hurricane radius for Vgmax with coriolis parameter
    ...
    %Gradient wind velocity at radius R
    VgR=(B(i,1)./Rhoa.*(RVmax(i,1)./Rgrid(:,:,i)).^B(i,1).*deltaP(i,1).*exp(-(RVmax(i,1)./Rgrid(:,:,i)).^B(i,1))).^0.5; %Gradient wind velocity at radius R without coriolis parameter
    %VgR=(B(i,1)./Rhoa.*(RVmax(i,1)./Rgrid(:,:,i)).^B(i,1).*deltaP(i,1).*exp(-(RVmax(i,1)./Rgrid(:,:,i)).^B(i,1))+(Rgrid(:,:,i).*f./2).^2).^0.5-(Rgrid(:,:,i).*f./2); %Gradient wind velocity at radius R with coriolis parameter
