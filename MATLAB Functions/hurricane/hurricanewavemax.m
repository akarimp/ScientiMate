function [Hsmax, Tmax, RVmax] = hurricanewavemax(xCenter, yCenter, Pc, Vmax, Rknown, VgRknown, Pn, Rhoa, VgToVCoeff, G, Vt, CalcMethod, RVmaxCalcMethod, dispout)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
switch nargin
    case 4
        Pn=101325; Rhoa=1.204; VgToVCoeff=0.8; G=0.88; Vt=0; CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 5
        Rhoa=1.204; VgToVCoeff=0.8; G=0.88; Vt=0; CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 6
        VgToVCoeff=0.8; G=0.88; Vt=0; CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 7
        G=0.88; Vt=0; CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 8
        Vt=0; CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 9
        CalcMethod='young13'; RVmaxCalcMethod='holland'; dispout='no';
    case 10
        dispout='no';
end
...
for i=1:length(xCenter(:,1))
    ...
    if strcmp(RVmaxCalcMethod,'rankine')==1
    ...
    elseif strcmp(RVmaxCalcMethod,'holland')==1

        %Calculating radius of maximum wind from known velocity and its radius
        fun = @(x)(VgRknown(i,1)-(B(i,1)/Rhoa*(x/Rknown(i,1))^B(i,1)*deltaP(i,1)*exp(-(x/Rknown(i,1))^B(i,1)))^0.5); %Hurricane radius for Vgmax without coriolis parameter
    ...
    elseif strcmp(RVmaxCalcMethod,'demaria')==1
    ...
    elseif strcmp(RVmaxCalcMethod,'slosh')==1
    ...
    elseif strcmp(RVmaxCalcMethod,'emanuel')==1
...
if strcmp(CalcMethod,'spm')==1

    RVmaxKm=RVmax./1000; %Convert m to km
    deltaPmmHg=deltaP.*0.00750061683; %Convert Pa to mmHg
    VtKt=Vt.*1.94384; %Convert m/s to knots
    wradhr=2.*pi./24; %Rotational frequency of the earth (Radian/hr)
    fradhr=2.*wradhr.*sin(deg2rad(yCenter)); %Coriolis parameter (Coriolis frequency) in rad/hr
    Umax=0.447.*(14.5.*deltaPmmHg.^0.5-RVmaxKm.*0.31.*fradhr);
    UR=0.865.*Umax+0.5.*VtKt;

    Hs=5.03.*exp(RVmaxKm.*deltaPmmHg./4700).*(1+0.29.*1.0.*VtKt./(sqrt(UR))); %Maximum significant wave height (m)
    Ts=8.6.*exp(RVmaxKm.*deltaPmmHg./9400).*(1+0.145.*1.0.*VtKt./(sqrt(UR))); %Maximum significant wave period (s)

    Hsmax=Hs; %Maximum significant wave height (m)
    Tmax=Ts; %Maximum significant wave period (m)


%Calculating hurricane wave propoerties based on Young (1988) 
elseif strcmp(CalcMethod,'young88')==1
    ...
    Rprime=22.5e3.*log10(RVmax)-70.8e3;
    ...
    Hsmax=Hs; %Maximum significant wave height (m)
    Tmax=Tp; %Maximum peak wave period (m)


%Calculating hurricane wave propoerties based on Young & Vinoth (2013) 
elseif strcmp(CalcMethod,'young13')==1
