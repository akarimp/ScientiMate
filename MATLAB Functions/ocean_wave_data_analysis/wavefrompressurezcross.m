function [Hs, Ts, Hz, Tz, Hrms, H, T, Eta, t] = wavefrompressurezcross(P, fs, h, heightfrombed, Kpmin, kCalcMethod, Rho, dispout)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
switch nargin
    ...
    case 4
        Kpmin=0.15; kCalcMethod='beji'; Rho=1000; dispout='no';
    case 5
...
for i=1:length(H(:,1))
    ...
    for j=xupcrossIndx(i,1):xupcrossIndx(i+1,1)
        EtawithKp(j,1)=EtaDetrended(j,1)/Kp(i,1);
    end
...
for j=1:xupcrossIndx(1,1)
    EtawithKp(j,1)=EtaDetrended(j,1)/Kp(1,1); 
end

%Data after very last wave, last Kp is used due lack of enough information
for j=xupcrossIndx(end,1):length(EtaDetrended(:,1))
    EtawithKp(j,1)=EtaDetrended(j,1)/Kp(end,1); 
end

Eta=EtawithKp; %Water surface elevation time series
...
for i=xupcrossIndx(1,1):xupcrossIndx(end,1)
    ...
    if i>1 & i<N
        ...
        if EtawithKp(i,1)>EtawithKp(i-1,1) & EtawithKp(i,1)>EtawithKp(i+1,1)
            ...
            if EtawithKp(i,1)>0
                ...
                if m==n
                    ...
                    m=m+1;
                    xmax(m,1)=t(i,1);
                    ymax(m,1)=EtawithKp(i,1);
                    xmaxIndx(m,1)=i;
                    
                elseif m~=0
                        
                    %Replacingthe old crest location with new one if the new one is larger
                    if EtawithKp(i,1)>ymax(m,1) & m~=0
        ...
        if EtawithKp(i,1)<EtawithKp(i-1,1) & EtawithKp(i,1)<EtawithKp(i+1,1)
            ...
            if EtawithKp(i,1)<0
                ...
                if n==m-1
                    ...
                    n=n+1;
                    xmin(n,1)=t(i,1);
                    ymin(n,1)=EtawithKp(i,1);
                    xminIndx(n,1)=i;
                    
                elseif n~=0
                        
                    %Replacingthe old crest location with new one if the new one is smaller
                    if EtawithKp(i,1)<ymin(n,1)
