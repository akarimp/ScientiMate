function [E, Pw, Sxx, Syy] = wavepower(h, H, T, Rho, kCalcMethod)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
%--------------------------------------------------------------------------
%Calculation of wave energy and power

E=1./8.*Rho.*9.81.*(H).^2; %Wave energy
Pw=Cg.*E; %Wave power
Sxx=E.*((2.*k.*h)./sinh(2.*k.*h)+1/2); %Wave radiation stress in x direction
Syy=E.*((k.*h)./sinh(2.*k.*h)); %Wave radiation stress in y direction
