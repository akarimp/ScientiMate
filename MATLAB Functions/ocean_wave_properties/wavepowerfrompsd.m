function [E, Pw] = wavepowerfrompsd(h, f, Syy, Rho, kCalcMethod)
%{
.. ++++++++++++++++++++++++++++++++YA LATIF++++++++++++++++++++++++++++++++++
.. +                                                                        +
.. + ScientiMate                                                            +
...
C=9.81./(2.*pi.*f).*tanh(k.*h); %Wave celerity
Cg=1./2.*C.*(1+(2.*k.*h)./sinh(2.*k.*h)); %Wave group celirity

%--------------------------------------------------------------------------
%Calculating wave energy and wave power

df=f(2,1)-f(1,1); %frequency intervals

Ei=1./8.*Rho.*9.81.*((4./sqrt(2).*((Syy.*df).^0.5)).^2); %Wave energy at each frequency, Hrms=Hm0/sqrt(2)
Pwi=Cg.*Ei; %Wave power at each frequency
Pwi=Pwi(isnan(Pwi)==0); %Removing NaN values

E=sum(Ei(:,1)); %Wave energy
Pw=sum(Pwi(:,1)); %Wave power
